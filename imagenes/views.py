...
def index(request, queryset, template = 'imagenes/index.html', extra_context = None):
	...
	imagenes_favoritas_obj = Ifavoritas.objects.filter(perfil=perfil_usuario, eliminado=False)
...
def marcar_portada_ajax(request):
	...
	if request.is_ajax():
		...
		if ya_tiene_portada:
			...
			portada_anterior = Ifavoritas.objects.filter(perfil=perfil_usuario, eliminado=False, portada=True)
			for p in portada_anterior:
				print p.imagen.id
				p.portada=False
				p.save()
			nueva_portada = Ifavoritas.objects.get(imagen=imagen, perfil=perfil_usuario, eliminado=False)
...
def favoritas(request, username):
	...
	if propio_usuario:
		...
		favoritas_usuario_visitante_obj = Ifavoritas.objects.filter(perfil = perfil_usuario_visitante, eliminado=False)
...
def denunciar(request):
	if request.is_ajax():
		...
		ya_denuncio = False
		ya_denuncio = Idenunciadas.objects.filter(imagen=imagen, perfil=perfil_usuario).exists()
		if ya_denuncio:
			print "ya denuncio"
			...
			if perfil_usuario.votos_recibidos > 10:
				...
				imagen.save()
				print "imagen denunciada id %s" %(imagen.id)
	...
	if hay_imagenes_denunciadas:
		...
		for i in imagenes_denunciadas_obj:
			...
			if Idenunciadas.objects.filter(imagen = i, perfil = perfil_usuario).exists():
				idenunciada_obj = Idenunciadas.objects.get(imagen = i, perfil = perfil_usuario)
				if idenunciada_obj.eliminado == False:
					accion_usr = ["", "-circle"]
				else:
					accion_usr =  ["-circle", ""]
...
def marcar_visto(request, imagen_id):
	...
	if Idenunciadas.objects.filter(imagen = imagen, perfil = perfil_usuario).exists():
		idenunciada_obj = Idenunciadas.objects.get(imagen = imagen, perfil = perfil_usuario)
		if idenunciada_obj.eliminado == False:
			imagen.denunciada -= 2
			idenunciada_obj.eliminado = True
			idenunciada_obj.save()
		elif idenunciada_obj.eliminado == True:
			pass
	else:
		imagen.denunciada -= 1
		idenunciada_obj = Idenunciadas(imagen=imagen, perfil=perfil_usuario, eliminado=True)
		idenunciada_obj.save()
